...
function findDuplicateTransactions(transactions) {
  ...
  function dateTime(time1, time2) {
    return (new Date(time1).getTime() - new Date(time2).getTime()) / 60000;
  }

  newTransaction = newTransaction.sort(function (a, b) {
    return a.id - b.id;
  });
  ...
  while (newTransaction.length) {
    let newArr = [];
    let copyTransaction = newTransaction.shift();

    newArr.push(copyTransaction);

    for (let i = 0; i < newTransaction.length; i++) {
      if (
        newTransaction[i].sourceAccount === copyTransaction.sourceAccount &&
        newTransaction[i].targetAccount === copyTransaction.targetAccount &&
        newTransaction[i].amount === copyTransaction.amount &&
        newTransaction[i].category === copyTransaction.category &&
        dateTime(newTransaction[i].time, copyTransaction.time) < 1
      ) {
        newArr.push(newTransaction[i]);

        copyTransaction = newTransaction[i];
    ...
    //   item.time = new Date(item.time * 60000).toISOString();
    // });
  }
